...
// "This and following": same rows, bounded below by an inclusive date. Date-granular, so two
// occurrences on the same day go together.
const getSeriesRowsFrom = db.prepare(
  'SELECT * FROM events WHERE series_id = ? AND date >= ? ORDER BY date, time'
);
...
// `from` (YYYY-MM-DD, validated at the route) narrows it to that date onward. Returns the count
// deleted (0 = unknown series / nothing at or after `from`).
export const deleteSeriesTx = db.transaction((series_id, from = null) => {
  const rows = from ? getSeriesRowsFrom.all(series_id, from) : getSeriesRows.all(series_id);
  for (const r of rows) {
    audit(r.id, 'delete', r);
